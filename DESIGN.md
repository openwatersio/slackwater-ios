...
2. The poller paginates both feedback collections to completion, checking each source ID against the private marker **before** fetching its build, image, or crash log. It signs a fresh short-lived Apple JWT for each authenticated request. For new submissions it uses the app-scoped list endpoints, downloads available screenshots promptly, and reads available crash logs, with an 8 MiB per-asset cap. Missing, expired, or oversized assets are recorded as private issue omissions, not as public or expiring links.
3. Before any write, the poller verifies that `GITHUB_REPOSITORY` is the configured **private** destination and that the repository API reports `private: true`. The workflow's `GITHUB_TOKEN` is scoped to this repository. It writes asset bytes to private repository files, then creates one private issue per Apple feedback ID. The public repository has no write path.
...
Define `source_key = <Apple app ID>:<screenshot|crash>:<Apple submission ID>`. The private repository owns a small `intake/<SHA-256(source_key)>.json` record containing the key, phase, asset paths, and eventual issue number—no raw feedback. A completed record skips the submission on later full scans.
...
The body begins with a delimited `<!-- testflight-feedback:v1 ... -->` JSON block containing `source_key`, Apple app/submission/build IDs, feedback type, app version/build number, submission time, device model, OS version, private asset paths (and legacy Release asset IDs), asset omissions, `triage_status: "needs-triage"`, `related_private_issue_numbers: []`, and `public_issue_url: null`. Below it, put the **verbatim** comment in a safe fenced code block and link only to private screenshot/log assets. Include an app-level App Store Connect link and the submission ID for lookup; Apple does not document a per-submission web URL in its API. Treat all text as untrusted when rendering. The future triage process can compare these records, redact PII, draft a reproduction-oriented public issue, and update the private links. This poller never writes to the public repo.
...
Public `slackwater-ios` repository:
    ...
    services/testflight-feedback/
      ...
      test/*.test.ts       # focused Apple, GitHub, and CLI checks
...
Private feedback repository: `.github/workflows/testflight-poll.yml`, Actions secrets/variables, `intake/` marker and asset files, and private issues. No database, webhook endpoint, Cloudflare deployment, dashboard, or public-issue agent.
...
3. Implement the private-repository guard, dedup marker phases, deterministic private asset files, and versioned private issue format. Add focused checks for duplicate runs, missing assets, private-repo guard, and ambiguous issue creation.
4. Run a manual private-repo workflow against real feedback, inspect sanitized logs and issue/assets, then enable the schedule. Document stuck-marker reconciliation and asset retention. Stop here; public triage remains a separate design and implementation.
