/**
 * Enrich the bundled currents.json in place with the station-corrections
 * gate→tide pairing (`tideReference`).
 *
 * Region presentation used to be fixed here too (M50): compass abbreviations
 * arrived title-cased ("Sse") and NOAA mixes statute miles into a marine app's
 * qualifiers. Both were symptoms of `station-corrections/src/clean.js`, fixed
 * upstream in v2.6.0 — the local normalizer is deleted and regeneration is
 * byte-identical. slackwater-web inherits the same fix by dependency bump.
 *
 * Two rungs, in order:
