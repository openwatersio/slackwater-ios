...
struct OnlineGateDetailView: View {
    ...
    /// The window around the next slack — looked up, not recomputed, so it
    /// can never disagree with the band the strip draws (#233).
    ...
    private func readout(_ window: ChsOnlineWindow, _ tl: TimelineData) -> some View {
            ...
            if let slack = nextSlack {
                ...
                VStack(alignment: .trailing, spacing: 1) {
                    ...
                    if let win = slackWin {
                        ...
                        // strip.
                        Text("for \(countdown(from: max(scrubTime, win.start), to: win.end)) @ \(formatSpeed(tl.slackThreshold, unit: speedUnit)) \(speedUnitLabel(speedUnit))")
                            .font(.caption.monospacedDigit())
                            .foregroundStyle(SN.foam.opacity(0.7))
