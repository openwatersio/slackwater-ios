...
| Station detail | the ~20 km box around each starred station **or station whose CHS model is downloaded** (`ChsFitService.queue` jobs at `.ready`); a box crossing ±180 splits into two packs, since `MLNCoordinateBounds` cannot express a wrapping span | z9–12 | created when starred, deleted when unstarred. Kept a self-contained module a config flip can turn off (owner ruling: the map is orientation, not navigation — cache size, not feature completeness, decides whether this tier survives; §7 gate 4 records the visual cost of dropping it) |
...
- The GPS tier holds cells only while location access is **authorized**: `LocationService` keeps its last fix after permission is revoked, and coverage around where the user used to be is not coverage they asked for.
- A failed `addPack` leaves `storage.packs` unchanged, so nothing would ask again — creation failures schedule their own retry rather than waiting for a favourite or a fix to move.
- The tiers surface in `OfflineManagerView` as one **Charts card** beside the CHS rows — areas ready over areas total, bytes held, and a "Refresh charts" control that re-validates every pack before departure. One card rather than a row per pack: the tiers are automatic, so the only questions a user has here are whether the map is ready and how to top it up. Pack progress and error notifications drive it. No new UI surface.
