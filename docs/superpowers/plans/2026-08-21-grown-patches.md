...
- **Licence:** raw bathymetry tiles live only under gitignored `tools/patch-pipeline/data/` — never committed, never bundled. Committed artifacts are derived cross-sections only. Bathymetry attribution (OGL – Canada for the GSC DEM; CHS NONNA verbatim notice iff NONNA data ships) lands in the same change as the first BC patch (Task 9). Owner ruling 2026-08-21: US = NBS Navigation T&E BAGs (BlueTopo absent in PNW); BC = GSC West Coast DEM 10 m primary, NONNA fallback; coverage source-of-record is openwaters.io/charts/seascape.
...
- [ ] **Step 2: Download GSC Canada West Coast Topo-Bathymetric DEM 10 m coverage** for the three passes into `data/tiles/<slug>/` + MANIFESTs (owner ruling: GSC DEM primary — NRCan, OGL – Canada, direct download via the open.canada.ca dataset the Seascape source list links; no account needed). Record the DEM's vertical datum from tile metadata (CGVD2013 ≈ MSL means `cd_to_mwl_m` ≈ 0 — verify, don't assume) and set `tile_value` from the metadata sign convention. If a throat's width guard or sensitivity sweep shows the DEM too smooth there, fall back to CHS NONNA-10 for that pass (portal account + licence acceptance + verbatim-notice obligation recorded in MANIFEST) and update the pipeline README's acquisition section to match reality.
...
- [ ] **Step 2:** Add bathymetry attribution to SettingsView's "Data & attribution" section, alongside the existing CHS clause-10 text. For the GSC DEM: OGL – Canada attribution ("Contains information licensed under the Open Government Licence – Canada" naming the GSC Canada West Coast Topo-Bathymetric DEM), plus one sentence in app voice: "Channel cross-sections for grown current patches are derived from this bathymetry; raw survey data is not included." Only if Task 8 fell back to NONNA for any pass: also add the **verbatim** notice required by the NONNA licence, copied exactly from the licence PDF recorded in that pass's MANIFEST — do not paraphrase the verbatim part.
