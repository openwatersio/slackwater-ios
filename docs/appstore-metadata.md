...
| Larger Text                  | Yes            | Chrome, the lead and the strip's pad scale with Dynamic Type (`SlackwaterTests/TypeScaleTests.swift`, `TimelineTests`). Chart labels stay fixed to the chart geometry by contract (`docs/scrubber.md` § 5). |
