...
| Larger Text                  | Not yet        | The lead, its pad and the pills scale, and chart labels stay fixed by contract (`docs/scrubber.md` § 5). The platform audit reports partial support on station names, distances, sun times and the `MonoLabel` eyebrows, and clipping on "MY LOCATION" and the moon tile; card-layout work, tracked in #438. |
