...
Refresh resolves each station by coordinates and advertised series within 3 km, uses 30-second request timeouts and 2.5-second pacing, removes duplicate boundary timestamps, validates finite samples and a complete sorted 15-minute grid across the requested bounds (plus Victoria's one off-grid decoder probe), and renames a validated temporary file over the prior recording only after success.
