...
Fast and unit modes disable Xcode's slow simulator diagnostics collection while
retaining assertions, screenshots, and `.xcresult` output. Full and live modes
keep diagnostics on failure.
...
The stubbed test covers target/device and diagnostics selection for every mode, fixture preparation order and failure, inherited environment clearing, invalid/conflicting arguments, one-device enforcement, and the lock probe/re-exec path. No Xcode build or simulator test was run; coordinator validation remains pending.
