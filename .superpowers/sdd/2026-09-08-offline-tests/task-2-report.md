...
- `-chsFixtureScenario provisional-final` pauses immediately after production `publishProvisional`; the UI runner releases `/tmp/slackwater-ui-<UUID>-after-provisional`.
...
- Fixture checkpoints are bounded to 60 seconds and names are scoped by a fresh UUID, preventing stale or cross-test releases.
...
- Final current render and next-day persistence: `testSeededFinalCurrentModelRendersOffline`.
